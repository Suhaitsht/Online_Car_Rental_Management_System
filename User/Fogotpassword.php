...
function sendMail($email,$reset_token)
{
    ...
    try {
        ...
        $mail->Password   = 'mxlmxmhkjqpoxjhz';                               //SMTP password
        $mail->SMTPSecure = 'tls';                           //Enable implicit TLS encryption
        $mail->Port       = 587;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
    
        //Recipients
        $mail->setFrom('suhaitsuhait58@gmail.com', 'Malkey Rent A Car');
        $mail->addAddress($email);     //Add a recipient
       
    
        //Content
        $mail->isHTML(true);                                  //Set email format to HTML
        $mail->Subject = 'Password rest link from Malkey Rent A Car';
        $mail->Body    = "We got a request link from you to create password<br>
        click the link below:<br>
        <a href='http://localhost/carbook-master/User/UpdatePassword.php?u_email=$email &reset_token=$reset_token'>
        Reset password </a>";
...
if(isset($_POST['reset']))
{
  $resetmail=$_POST['resetmail'];
    $sql="SELECT * FROM `users` WHERE `U_email`='$resetmail'";
    //echo"$sql";
    ...
    if($result)
    {
        if(mysqli_num_rows($result)==1)
        {
            ...
            $update = "UPDATE `users` SET `reset_token`='$reset_token',`resettokenexpaire`='$date' WHERE `U_email` ='$resetmail'";
    ...
    <div class="row px-3">
      <div class="col-lg-6 col-xl-5 card flex-row mx-auto px-0">
        ...
        <div class="card-body">
          ...
          <form action ="Fogotpassword.php" method="POST" id= "reset">
          <div class="form-box px-3">
            <div class="form-input">
              <span><i class="fa fa-envelope-o"></i></span>
              <input type="email" name="resetmail" placeholder="Enter your Email Address" required>
            </div>
